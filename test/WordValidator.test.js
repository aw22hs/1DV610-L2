...
describe('WordValidator', () => {
  ...
  describe('constructor', () => {
    ...
    test('should throw an error for a word without any letters', () => {
      expect(() => new WordValidator('12345')).toThrowError('The submitted ' +
        'word does not have the right format.')
    })
    ...
    test('should not throw an error for a word with numbers and special ' +
      'characters', () => {
      expect(() => new WordValidator("test-1.2:3/4'5")).not.toThrow()
    })

    test('should throw an error for a word without letters but with special ' +
      'characters', () => {
      expect(() => new WordValidator("-.:/'")).toThrowError('The submitted ' +
        'word does not have the right format.')
    })

    test('should throw an error for a word without letters but with special ' +
      'characters', () => {
      expect(() => new WordValidator('a!c')).toThrowError('The submitted ' +
        'word does not have the right format.')
    })
  })
})
