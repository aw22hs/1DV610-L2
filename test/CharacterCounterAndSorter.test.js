...
describe('get sorted characters', () => {
  ...
  test('should return an object with one character plus character count when there is one character', () => {
    const characterCounterAndSorter = new CharacterCounterAndSorter(['A'])
    expect(characterCounterAndSorter.getSortedCharacters()).toEqual({ A: 1 })
  })

  test('should return an objects of words sorted in alphabetical order plus word count when there are several words',
    () => {
    const characterCounterAndSorter = new CharacterCounterAndSorter(['This', 'is', 'a', 'sentence', 'This', 
      'is', 'yet', 'another', 'sentence', 'And', 'this', 'is', 'a', 'third', 'one'])
